...
```
skill open-spec 修改 UserController 的 getUser 接口，增加返回用户邮箱字段
```
...
完整走 `skill open-spec` → `skill open-apply` → `skill open-review` → `skill open-archive` 流程。
