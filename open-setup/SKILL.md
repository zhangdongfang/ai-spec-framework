...
分析工程结构、依赖、分层模式、编码习惯，填充 `rules/` 下的配置文件。
...
```
[open-setup v2.0] 分析项目工程结构和编码风格，填充 rules/ 配置文件
[加载] 构建文件: ✅/❌ · 项目目录: <路径>
...
```
rules/        ← 存放 project-rules.md + domain-rules.md
changes/      ← 存放每次变更的 spec.md + tasks.md + log.md
archives/     ← 存放已完成变更的归档
knowledge/    ← 存放踩坑记录、技术决策等沉淀知识
```

### Step 3: 填充 rules/project-rules.md
...
### Step 4: 填充 rules/domain-rules.md
...
- Step 5 必须执行，确保 AGENTS.md 与 rules 同步
- Step 5 必须执行，确保 AGENTS.md 与 rules 同步
- Step 5 必须执行，确保 AGENTS.md 与 rules 同步
- Step 5 必须执行，确保 AGENTS.md 与 rules 同步
