...
```
[open-setup v2.1 | <项目类型> | 构建文件:✅/❌] 开始初始化项目上下文
[待生成] project-rules.md · domain-rules.md · AGENTS.md
```
...
5. 扫描外部服务依赖（搜索 HTTP Client 调用、MQ 消费/生产、RPC 调用、数据库连接配置），识别本服务的上下游拓扑
...
| 方向 | 服务/组件 | 协议 | 用途 | 配置位置 |
|------|----------|------|------|---------|
| 下游 | （如 order-service） | HTTP/RPC | （如"查询订单"） | （配置文件路径） |
| 下游 | （如 MySQL） | JDBC | 主数据库 | （配置文件路径） |
| 下游 | （如 Redis） | Redis | 缓存/分布式锁 | （配置文件路径） |
| 上游 | （如 gateway） | HTTP | 被谁调用 | — |
...
| 方向 | 服务/组件 | 协议 | 用途 | 配置位置 |
|------|----------|------|------|---------|
| （扫描代码中的 HTTP Client/DB/MQ/RPC 调用填充） | | | | |
...
| 方向 | 服务/组件 | 协议 | 用途 | 配置位置 |
|------|----------|------|------|---------|
| （扫描代码中的 HTTP Client/DB/MQ/gRPC 调用填充） | | | | |
...
- project-rules.md: ✅/❌
- domain-rules.md: ✅/❌
- AGENTS.md: ✅/❌ ← 如果 ❌，必须立即执行 Step 5 生成
```

如果 AGENTS.md 为 ❌，不得结束 skill，必须立即执行 Step 5。
